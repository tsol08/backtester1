...
class SubsetEqualWeight:
    ...
    PEAD가 그렇다. 한때 SUE를 가진 종목이 유니버스를 연 5.53%(t 4.12) 이기는 것으로
    보였는데, 그건 DART 배치 캐시 충돌로 591종목의 재무가 비어 '재무가 있는 종목'이
    옛 대형주 집단이 된 탓이었다. 고친 뒤에는 연 1.47%(t 1.31)로 유의하지 않다.
    그래도 기준은 후보 집합으로 둔다 - 후보 집합 효과가 0이라는 보장은 없고, 있으면
    검정한 적 없는 것을 성과로 파는 셈이 되기 때문이다.
    """

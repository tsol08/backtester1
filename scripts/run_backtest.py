...
def main() -> None:
    ...
    print(
        "\n  두 기준을 같이 보이는 이유: 전략이 고르는 후보 집합이 유니버스와 다르면,"
        "\n  유니버스를 기준으로 삼는 순간 '후보에 든 것만으로 생기는 차이'까지 전략"
        "\n  성과로 잡힌다. 한때 그 차이가 연 5.53%(t 4.12)로 보였으나 DART 배치 캐시"
        "\n  충돌이었고, 고친 뒤에는 연 1.47%(t 1.31)로 유의하지 않다."
    )
